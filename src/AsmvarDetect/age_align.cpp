...
int main(int argc,char *argv[]) {
	...
	cerr << "# [INFO] Target fa and Query fa are loaded successfully." 
		 << local_time();
	
	vector<VarReg> varReg = LoadVariantRegion ( ops.varInfile.c_str() );
	cerr << "# [INFO] Variant regions are loaded successfully."        
		 << local_time();
	if  (varReg.size() == 0) cerr << "[WARNING] The variant list is empty!\n";
	for ( size_t i(0); i < varReg.size(); ++i ) {

		if ((toupper(ops.selectTarId) != "ALL") 
			&& (toupper(ops.selectTarId) != toupper(varReg[i].target.id))) 
			continue;

		if ( !tarSeq.fa.count(varReg[i].target.id) ) { 
			cerr << "# [ERROR] "    << varReg[i].target.id << " is not in " 
				 << ops.tarFaInfile << "\n";
			exit(1);
		}
		if ( !qrySeq.fa.count(varReg[i].query.id) ) { 
            cerr << "# [ERROR] " << varReg[i].query.id << " is not in " 
				 << ops.qryFaInfile << "\n";
            exit(1);
        }
		Sequence* old_seq1 = new Sequence(tarSeq.fa[varReg[i].target.id], 
										  varReg[i].target.id, 1, false);
		Sequence* old_seq2 = new Sequence(qrySeq.fa[varReg[i].query.id], 
										  varReg[i].query.id , 1, false);
