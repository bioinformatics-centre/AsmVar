...
sub SV_SummaryReport {
    ...
    while (<$fh>) {
        ...
        if ($isgatkvcf) {
        # Specific for the VCF file generated by GATK
            SummaryGATK(\%summary,
                        \%allsvtype,
                        \%sizeSpectrum,
                        @col[3,4],
...
sub SummaryGATK {
    ...
    for (my $i = 0; $i < @samples; ++$i) {
        ...
        my ($svtype, $svsize) = AsmvarVCFtools::GetGATKSVtypeAndSizeForSample(
                    $seq[0],    # Ref-sequence
                    $seq[$ai]); # Alt-sequence
