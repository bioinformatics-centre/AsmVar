...
die "perl $0 <-split> -svd [svdfile] -q [queryFa] -t [targetFa] -n [sampleName] -p [ageAlg_parameter] -s [shell dir] -o [out_dir]\n" if ( !defined $svdfile or !defined $queryFa or !defined $targetFa or !defined $shdir or !defined $outdir or !defined $nameprefix or !defined $ageAlg_parameter );
system ( "mkdir -p $outdir" ) if ( ! -d $outdir );
system ( "mkdir -p $shdir"  ) if ( ! -d $shdir  );
$outdir     = abs_path($outdir); $outdir =~ s#/net/home##;
$shdir      = abs_path($shdir ); $shdir  =~ s#/net/home##;
...
sub generate_age_sh{
	...
	while(<I>){
		next if /^#/;
		chomp;
		my ($tchr,$tvs,$tve,$tvsize,$tvn,$tlen,$qchr,$qvs,$qve,$qvsize,$qvn,$qlen,$str,$ms,$mip,$type)=split(/\s+/,$_);  
		...
		$targetId{$tchr} = 1;
		print O join "\t", ( $tchr,$ts,$te,$qchr,$qs,$qe,"$type:$str:$ms:$mip:$tchr:$tvs:$tve:$tvsize-$qchr:$qvs:$qve:$qvsize" ), "\n";
	}
